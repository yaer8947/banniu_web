...
YX.fn.team = function () {
	...
	//try
	this.$teamInfoContainer.delegate('#teamMemberList .m_infor', 'click', this.showMemberInfor.bind(this))
...
YX.fn.showTeamView = function(type,teamType) {
	...
	this.$createTeamContainer.load('./createTeam.html', function() {
		...
		// }
		//try
		var keyList= that.cache.getUsersKeyList();
		for (var i = 0,l = keyList.length; i < l; ++i) {
			tmp += appUI.buildTeamMemberList(list[keyList[i]])
		}
		$addUserUl.html(tmp)
		if (type !== 0) {
...
YX.fn.checkedUser =  function(o) {
	...
	if (!$checkIcon.hasClass('cur2')) {  // 已是群成员，无法选择
		...
		//try
		var str = '<li data-account="' + account + '" data-account="' + name + '" data-icon="' + icon + '"><img src="' + icon + '" width="56" height="56"/><p class="name">' + name + '</p></li>'
	...
	this.$teamInfoContainer.load('./teamInfo.html', function() {
		...
		var showMember = function(){	
			...
			for(var i = 0; i<members.length; i++){
				...
				if(!that.cache.getUserFromId(members[i].account)){
		...
		var showUI = function(){
			...
			for (var i = 0, l = members.length; i < l; ++i) {
				var member = members[i],
					...
					//try
					user= that.cache.getUserFromId(account),//?that.cache.getUserFromId(account).avatar:"",
				...
				html += '<li data-account="' + account + '"><a href="javascript:"><img class="m_infor" data-account="' + account + '" src="'+avatar+'"/>'
...
//try
YX.fn.showMemberInfor= function (event) {
	var ev = event || window.event,
		$target = $(ev.target),
		account = $target.attr('data-account')
	// alert(account);

	this.showInfo(account, "p2p");
}
