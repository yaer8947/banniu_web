...
//try
    ...
    var $node = this.$myInfo.data({info:user})
    $node.find(".u-icon").attr('src', user.userIcon+"")
    $node.find(".j-nick").text(user.workNick)
    $node.find(".j-nickname").text(user.workNick)
    var avatarSrc=""
    if(user.gender&&user.gender!=="unknown"){
        avatarSrc = 'images/'+user.gender+'.png'
        $node.find(".j-gender").removeClass("hide")
...
YX.fn.showInfo = function (account,type) {
	if(type=="p2p"){
        ...
        //try
        var user = this.cache.getUserFromId(account)
...
YX.fn.showInfoInChat = function(account){
    ...
    //try
        user = yunXin.cache.getUserFromId(account)
    if(account==userUID){
        return
    }
...
YX.fn.showInfoBox = function(user,type){
    ...
    //try
    this.iPreviousWidth= this.iframeWidth
     if(user.id === userUID){
         this.showMyInfo()
    ...
    if(type==="team"){
        ...
        if(isManager){
            ...
            //try
            teamMute = this.cache.getTeamMemberInfo(user.id,this.crtSessionAccount).mute
    ...
    //try
    //user= this.cache.getUserFromId(user.account);
    var isFriend = this.cache.isFriend(user.id)
    var inMutelist = this.cache.inMutelist(user.id)
    var inBlacklist = this.cache.inBlacklist(user.id)
    var $node = this.$personCard.data({account:user.id,inMutelist:inMutelist?true:false,inBlacklist:inBlacklist?true:false,teamMute:teamMute})
    $node.find(".u-icon").attr('src', user.userIcon?user.userIcon:"images/default-icon.png")
    $node.find(".j-nickname").text("昵称："+user.workNick)
    $node.find(".j-nick").text(this.getNick(user.id))
    var avatarSrc =""
    if(user.gender&&user.gender!=="unknown"){
        avatarSrc = 'images/'+user.gender+'.png'
        $node.find(".j-gender").removeClass("hide")
    ...
    $node.find(".j-gender").attr('src',avatarSrc)
    $node.find(".j-username").text("帐号："+user.id)
    $node.find(".j-birth").text(user.birth ===undefined?"--":user.birth)
    $node.find(".j-tel").text(user.mobile ===undefined?"--":user.mobile)
    $node.find(".j-email").text(user.email ===undefined?"--":user.email)
    $node.find(".j-sign").text(user.sign ===undefined?"--":user.sign)
...
YX.fn.hideInfoBox = function(){
    ...
    this.$mask.addClass('hide')
    this.iframeWidth= IFrameResize(this.iPreviousWidth, 0).width
