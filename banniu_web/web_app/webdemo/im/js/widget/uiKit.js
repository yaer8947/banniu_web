...
/***/ function(module, exports, __webpack_require__) {
	...
	uiKit.OrganizationTree= __webpack_require__(3);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var SessionList = function(options){
		var parent = options.parent,
			...
			// cbrClickList = options.onrclickitem||function(account,type){console.log('account:'+account+'---type:'+type);},
...
// /***/ },
	 ...
	 * @property {Object} data 消息数据 data.friends 好友数据 data.friends  data.userinfo 用户信息 data.account 当前用户账号
	 */
	var OrganizationTree = function(options){
		var parent = options.parent,
			data = options.data,
			cbClickList = options.onclickitem||function(account,type){console.log('account:'+account+'---type:'+type);},
			cbClickPortrait = options.onclickavatar||function(account,type){console.log('account:'+account+'---type:'+type);};
		...
		this.provider = options.infoprovider;
		this._body = document.createElement('ul');
		this._body.className = options.clazz||"m-panel" +" j-organization";
	 ...
	 * @return {Void}
	 */
	OrganizationTree.prototype.inject = function(node){
		var injectNode = util.getNode(node);
		injectNode.innerHTML = "";
		injectNode.appendChild(this._body);
	};
	 ...
	 * @return {Void}
	 */
	OrganizationTree.prototype.update = function(data){
		var html="",
			list = data.friends,
			info;
		// for (var i = 0; i < list.length; i++) {
...
//try
		for (var i in list) {
			if (list.hasOwnProperty(i)) { //filter,只输出man的私有属性
				info = this.provider(list[i],"friend");
				var ava= info.avatar?info.avatar:"images/default-icon.png";
				if (i !== ACCOUNT) {
					html += ['<li class="panel_item '+(info.crtSession===info.target?'active':'')+'" data-scene="p2p" data-account="' + info.account + '">',
						'<div class="panel_avatar"><img class="panel_image" src="'+ ava +'"/></div>',
						'<div class="panel_text">',
						'<p class="panel_single-row">'+info.nick+'</p>',
						'</div>',
						'</li>'].join("");
	 ...
	 * @return {void}
	 */
	OrganizationTree.prototype.destory = function(){
		//预留
	};
