var appUI = {
    	...
    	if(msgs.length===0){
    		...
    		for (var i = 0, l = msgs.length; i < l; ++i) {
			 	var message = msgs[i],
                    ...
                    //try
                    user = cache.getUserFromId(message.from);
				//消息时间显示
			 	if(i == 0){
    ...
    updateChatContentUI:function(msg,cache){
	 	var lastItem =$("#chatContent .item").last(),
        	msgHtml="",
        	user =cache.getUserFromId(msg.from);//cache.getUserById(msg.from);//try
		...
		if (message.attach && message.attach.type) {
            ...
            if(type==="tip"){
    			...
    			msgHtml = ['<div data-time="'+ message.time +'" data-id="'+ message.idClient +'" id="'+ message.idClient +'" data-idServer="'+ message.idServer +'" class="item item-' + buildSender(message) + '">',
                    ...
                    //try
                            '<img class="img j-img" src="'+avatar+'" data-account="' + from + '"/>',
                            showNick?'<p class="nick">' + getNick(from)  + '</p>':'',
    ...
    buildCloudMsgUI:function(msg,cache){
        ...
        for (var i = len - 1; i >= 0; --i) {
            ...
            msgHtml += this.makeChatContent(message,cache.getUserFromId(message.from))//cache.getUserById(message.from))//try
        }
        return msgHtml;
    },
    ...
    buildTeamMemberList:function(list){
        ...
        var icon= list.userIcon?list.userIcon:"images/default-icon.png";
        return ['<li data-icon="' + icon + '" data-account="' + list.id + '" data-account="' + list.workNick + '">',
            '<i class="icon icon-radio"></i>',
            '<img src="'+icon+'">',
            '<span class="name">' + list.workNick + '</span>',
            '</li>'].join('');
    },
    ...
    buildCustomSysMsgs:function(data,cache){
        ...
        for(var i = 0;i<data.length;i++){
            ...
            if(scene==="p2p"){
                nick = getNick(from);
                avatar = cache.getUserFromId(from)?cache.getUserFromId(from).userIcon:"images/default-icon.png";//getAvatar(cache.getUserById(from)?cache.getUserById(from).avatar:"");//try
            }else{
