...
var Cache = (function(){
	...
	*公司数据（个人数据， 公司组织架构数据， 公司群数据）
 	*/
	Cache.prototype.loadXMLDoc= function(url,cfunc)
	{
		var xmlhttp;
		...
		xmlhttp.onreadystatechange=cfunc.bind(this, xmlhttp);
		xmlhttp.send();
	}

	Cache.prototype.initData= function(){
		this.loadXMLDoc('http://localhost:63342/NIM_Web/webdemo/im/js/data.json',function(xmlhttp)
		{
			if (xmlhttp.readyState==4 && xmlhttp.status==200)
			{
				var dataObj = JSON.parse(xmlhttp.responseText);
				this.setData(this.adeptsKeyList, this.adeptsList,  dataObj.allDeptList);
				this.setData(this.usersKeyList, this.usersList,
					this.setUserData(dataObj.allUserList, "@1e_80w_80h_1c_0i_1o_80Q_1x_100-1ci.png"));//dataObj.allUserList);
			}
		});

		this.loadXMLDoc('http://localhost:63342/NIM_Web/webdemo/im/js/tribe.json',function(xmlhttp)
		{
			if (xmlhttp.readyState==4 && xmlhttp.status==200)
			{
				this.setData(this.tribesKeyList, this.tribesList,  JSON.parse(xmlhttp.responseText).projectList);
				this.initNim()
			}
		});

		this.loadXMLDoc('http://localhost:63342/NIM_Web/webdemo/im/js/userData.json',function(xmlhttp)
		{
			if (xmlhttp.readyState==4 && xmlhttp.status==200)
			{
				this.userData=  JSON.parse(xmlhttp.responseText);
			}
		});
	}

	Cache.prototype.setUserData= function (userList, data) {
		for (var i = userList.length - 1; i >= 0; i--) {
			var strIcon= userList[i].userIcon.toString();
			if (strIcon==null||strIcon=="")
				continue;
			var n= strIcon.indexOf("?");
			userList[i].userIcon= strIcon.substring(0,  n>0?n:strIcon.length)+data;

		};
		return userList;
	}
	
	Cache.prototype.setData= function (keyList, obj, data) {
		var item;
		for (var i = data.length - 1; i >= 0; i--) {
			item = data[i];
			keyList.unshift(item.id);
			obj[item.id] = item;
		};
	}

	Cache.prototype.getAdeptsLst = function () {
		return this.adeptsList;
	};

	Cache.prototype.getUsersList = function () {
		return this.usersList;
	};

	Cache.prototype.getUsersKeyList = function () {
		return this.usersKeyList;
	};

	Cache.prototype.getTribesList = function () {
		return this.tribesList;
	};

	Cache.prototype.getUserData= function () {
		return this.userData;
	}

	Cache.prototype.getUserFromId = function (id) {
	...
	Cache.prototype.getTribeFromId = function (id) {
		if(this.tribesList[id]){
			return this.tribesList[id];
		}
		return false;
	}
